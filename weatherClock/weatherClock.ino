...
//const byte charTable[] = {
  ...
  0B00000000, 0B00000000, 0B00000000, 0B00000000, 0B00000000, 0B00000000, 0B00000000, 0B00000000,
  0B00000000, 0B00000000, 0B00000000, 0B00000000, 0B00000000, 0B00000000, 0B00000000, 0B00000000,
  0B00000000, 0B00000000, 0B00100010, 0B00000000, 0B00000000, 0B00000000, 0B00000000, 0B00100000,
  0B00111001, 0B00001111, 0B00000000, 0B00000000, 0B00000000, 0B01000000, 0B10000000, 0B00000000,
  0B00111111, 0B00000110, 0B01011011, 0B01001111, 0B01100110, 0B01101101, 0B01111101, 0B00000111,
  0B01111111, 0B01101111, 0B01000000, 0B00000000, 0B00000000, 0B01001000, 0B00000000, 0B00000000,
  0B00000000, 0B01110111, 0B01111111, 0B00111001, 0B00111111, 0B01111001, 0B01110001, 0B00111101,
  0B01110110, 0B00000110, 0B00011110, 0B01110101, 0B00111000, 0B00110111, 0B01010100, 0B00111111,
  0B01110011, 0B01100111, 0B01010000, 0B01101101, 0B01111000, 0B00111110, 0B00111110, 0B00111110,
  0B01110110, 0B01101110, 0B01011011, 0B00000000, 0B00000000, 0B00000000, 0B00000000, 0B00001000,
  0B00000000, 0B01110111, 0B01111100, 0B01011000, 0B01011110, 0B01111001, 0B01110001, 0B00111101,
  0B01110100, 0B00000100, 0B00011110, 0B01110101, 0B00000110, 0B00110111, 0B01010100, 0B01011100,
  0B01110011, 0B01100111, 0B01010000, 0B01101101, 0B01111000, 0B00011100, 0B00011100, 0B00011100,
  0B01110110, 0B01101110, 0B01011011, 0B00000000, 0B00000000, 0B00000000, 0B00000000, 0B00000000,
};
...
void loop() {
  ...
  //msg="Temp:" + String(cTemp, 2) + "*C";
  //String("Temp:" + String(cTemp, 2) + "*C").toCharArray(cStr, 17);
  //msg_send(cStr);
  msgg();
  delay(50000);
...
void setChar( char value) {
  ...
  //Serial.write(pgm_read_byte_near(charTable + index));

  //swSer.write(charTable[index]);
  swSer.write(pgm_read_byte_near(charTable + index));
}
...
void dotChar(char value){
  ...
  //swSer.write(charTable[index]);
  swSer.write((pgm_read_byte_near(charTable + index))|0B10000000);
  Serial.print((pgm_read_byte_near(charTable + index))|0B10000000);
  return;
}

void msg_send(char msg[]) {
  //Serial.print(msg);
  msg_index = strlen(msg) - 1;
  //Serial.print(msg_index);
  if (msg_index < 17) {
    swSer.write(0x0D);
    /*for (int i=msg_index;i>=0;i--){
      setChar(msg[i]);
      }*/
    uint8_t t = 15 - msg_index;
    for (int i = 0; i < t / 2; i++) {
      swSer.write(0x00);
    }
    for (int i = msg_index; i >= 0; i--) {
      //update code updated for dot chracters
      if (msg[i] == '.') {
        dotChar(msg[--i]);
        //i--;
      }
...
void msgg(){
    ...
    //msg="Temp:" + String(cTemp, 2) + "*C";
  String  mesgg=String("       ")+weathermain;
  String(mesgg).toCharArray(cStr, 17);
  msg_send(cStr);
}
